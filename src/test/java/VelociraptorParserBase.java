...
public abstract class VelociraptorParserBase extends Parser {
    protected VelociraptorParserBase(TokenStream input) {
        super(input);
    }
    ...
    protected boolean lineTerminatorAhead() {
        ...
        if (ahead.getChannel() != VelociraptorLexer.HIDDEN) {
            // We're only interested in tokens on the HIDDEN channel.
            return false;
        }

        if (ahead.getType() == VelociraptorLexer.TERMINATOR) {
            // There is definitely a line terminator ahead.
            return true;
        }

        if (ahead.getType() == VelociraptorLexer.WS) {
        ...
        return (type == VelociraptorLexer.COMMENT && (text.contains("\r") || text.contains("\n"))) ||
                (type == VelociraptorLexer.TERMINATOR);
    }
    ...
    protected boolean noTerminatorAfterParams(int tokenOffset) {
        ...
        if (stream.LT(tokenOffset).getType() == VelociraptorLexer.L_PAREN) {
            // Scan past parameters
            while (leftParams != rightParams) {
                tokenOffset++;
                valueType = stream.LT(tokenOffset).getType();

                if (valueType == VelociraptorLexer.L_PAREN) {
                    leftParams++;
                } else if (valueType == VelociraptorLexer.R_PAREN) {
                    rightParams++;
                }
            }
